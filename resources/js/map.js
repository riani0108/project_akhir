...
function clientPopupTemplate(clientId) {
    return `
        <div class="client-popup">
            <strong>Client ID:</strong> ${clientId}<br>
            <small>Klik marker untuk konfigurasi</small>
            <br><br>
            <button onclick="removeClient(${clientId})"
...
const antennaDB = {
    ...
    // ===== CLIENT =====
    nanobeam_16: {
        name: "NanoBeam 16 dBi",
        type: "client",
        gain: 16,
    },
    litebeam_23: {
        name: "LiteBeam 23 dBi",
        type: "client",
        gain: 23,
    },
    dish_30: {
...
let selectedClientAntenna = antennaDB["nanobeam_16"];
...
let ptmpLines = [];
let ptmpSector = null;
let ptmpBeamwidth = 90; // default sector 90°
let ptmpRadiusKm = 5; // jangkauan sector
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    let map; // akan diinisialisasi nanti
...
`;
            ...
            `,
                        ...
                        group.list.forEach((a) => {
                            if (
                                ...
                                beamwidth = 90;
                            if (a.jenis_antenna?.toLowerCase().includes("omni"))
                                beamwidth = 120;
                        });

                        const color = getSectorColor(distanceKm);
                        createSector(
                            ap,
                            azimuth,
                            beamwidth,
        ...
        routingMachine = L.Routing.control({
            ...
            .on("routesfound", function (e) {
                ...
                for (let i = 0; i <= 60; i++) {
                    const t = i / 60;
                    const lat =
                        startLatLng.lat + (endLatLng.lat - startLatLng.lat) * t;
                    const lng =
                        startLatLng.lng + (endLatLng.lng - startLatLng.lng) * t;
    ...
    function openClientPanel(clientId) {
        ...
        if (header) {
            ...
            span.textContent = `Client ${clientId}`;
    ...
    function updatePtMPLines() {
        ...
        clientMarkers.forEach((client, index) => {
            ...
            line.on("click", (e) => {
                ...
                showAntennaPanel(result.rxPower);
            });
    ...
    function savePtMPState() {
        ...
        const state = {
            ...
            apAntenna: selectedAPAntenna,
            clientAntenna: selectedClientAntenna,
        };
    ...
    function clearPtMP() {
        ...
        ptmpLines.forEach((l) => map.removeLayer(l));
        ptmpSector && map.removeLayer(ptmpSector);
    ...
    function setActiveUIButton(activeBtn) {
        document
            .querySelectorAll("#ptmp-ui button")
    ...
    document.getElementById("ap-antenna").addEventListener("change", (e) => {
        selectedAPAntenna = antennaDB[e.target.value];
    ...
    function restorePtMPState() {
        ...
        selectedAPAntenna = state.apAntenna;
        selectedClientAntenna = state.clientAntenna;
        ptmpBeamwidth = selectedAPAntenna.beamwidth;
